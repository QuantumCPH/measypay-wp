...
.nivo-caption p, .nivo-caption h1, .nivo-caption h2,.nivo-caption h3, .nivo-caption h4{
    padding:0;
	margin:0;
}
.nivo-caption p{
...
.nivo-controlNav a {
	background:url("<?php echo WP_PLUGIN_URL . "/wp-nivo-slider/"; ?>images/icon.png") no-repeat scroll 0 0 transparent;
	border:0 none;
	display:block;
	float:left;
	height:11px;
...
<?php 
	...
	icl_object_id($category,"category",true);
	//echo $category;
?>
<?php query_posts( 'cat='.$category.'&posts_per_page=$n_slices' ); if( have_posts() ) : ?>
   <div id="slider">  
